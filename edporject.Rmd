...
Upon inspecting the qqnorm plots of the runtime of all programming languages on the same machine, we discovered that, with the exception of C and C++, the runtime of most languages tend to not follow a normal distribution. Note: The sample size is too small to meaningfully apply Shapiro-Wilk test
_todo: Insesrt qqplots_

To address this, we run the program 5 times per sample for each programming language, and repeating the process 30 times. By the Central Limit Theorem (CLT), we may assume the distribution of sample means to be normal [2]. The mean of the distribution of sample means is then an unbiased estimator for the true run time of each programming language[2], which we take as a single observation.
