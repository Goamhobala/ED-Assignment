...
Upon inspecting the qqnorm plots of the runtime of all programming languages on the same machine, we discovered that the runtime of languages tend to not follow a normal distribution, even when the sample size is big (n = 100)
...
```
After we have applied the procedure described above, we obtained the following result:
...
plot <- ggplot(pilot.long, aes(x=language, y=runtime, group=Hardware, color=Hardware))+
  geom_point()+
  geom_line()+
